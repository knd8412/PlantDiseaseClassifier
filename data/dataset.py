...
def create_dataloaders(
    ...
    if use_weighted_sampling:
        if sample_weights is None:
            raise ValueError(
                "sample_weights must be provided when use_weighted_sampling=True"
            )
...
def load_dataset_and_dataloaders(
    dataset_size="medium", config_path="configs/train.yaml"
):
    """
    Full pipeline:
      - Load config
    ...
    train_transforms = get_transforms(
        image_size=image_size, train=True, normalize=normalize, augment=augment
    )

    val_transforms = get_transforms(
        image_size=image_size, train=False, normalize=normalize, augment=False
    )
