...
def get_transforms(image_size=224, train=True, normalize=True, augment=True):
    ...
    if normalize:
        color_transform.append(transforms.Normalize(mean=IMAGENET_MEAN, std=IMAGENET_STD))
        grayscale_transform.append(transforms.Normalize(mean=GENERIC_MEAN, std=GENERIC_STD))
        segmented_transform.append(transforms.Normalize(mean=GENERIC_MEAN, std=GENERIC_STD))
