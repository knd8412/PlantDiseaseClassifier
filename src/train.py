...
def stratified_split(dataset, labels, val_size=0.15, test_size=0.15, seed=42):
    ...
    sss2 = StratifiedShuffleSplit(n_splits=1, test_size=val_size/(1.0 - test_size), random_state=seed)
...
def main():
    ...
    labels = [int(item.get("label") if "label" in item else item.get("labels")) for item in full]
    num_classes = len(set(labels))
    print(f"[Data] Classes: {num_classes}, Samples: {len(full)}")

    # Stratified split
    train_idx, val_idx, test_idx = stratified_split(full, labels, cfg.data["val_size"], cfg.data["test_size"], seed=cfg.seed)
    ...
    for epoch in range(1, cfg.train["epochs"] + 1):
        ...
        if va_acc > best_val_acc + 1e-4:
            ...
            if epochs_no_improve >= patience:
                print(f"[EarlyStopping] No val acc improvement for {patience} epochs, stopping.")
                break

    # Save metrics
    with open(os.path.join("outputs", "metrics.json"), "w") as f:
        json.dump({"best_val_acc": best_val_acc, **history}, f, indent=2)

    print(f"[Done] Best validation accuracy: {best_val_acc:.4f}. Weights saved to {best_path}")
